...
class Session {
    ...
    void login(string username, string password) {
        ...
        auto res = post(login_url, encode("user=" ~ username ~ "&password=" ~ password ~ "&version=" ~ launcher_version));
    ...
    void join(string server_hash) {
        ...
        auto res = get(encode(join_server_url ~ "?user=" ~ username ~ "&sessionId=" ~ session_id ~ "&serverId=" ~ server_hash));
        debug writefln("(SESSION) Server returned: \"%s\"", res);
    ...
    void keep_alive() {
        ...
        auto res = get(encode(keep_alive_url ~ "?name=" ~ username ~ "&session=" ~ session_id));
        debug writefln("(SESSION) Server returned: \"%s\"", res);
    }
}