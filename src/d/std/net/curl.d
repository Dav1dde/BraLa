...
struct HTTP
{
    ...
    */
                                                 ...
                                                 in char[] value) callback)
    {
        ...
        auto dg = delegate void(in char[] header)
        {
            if (header.empty)
            {
                // header delimiter
            ...
            if (header.startsWith("HTTP/"))
            {
                ...
                {
                    p.status.majorVersion = to!ushort(head.munch("01234567890"));
                    head.munch(". ");
                    p.status.minorVersion = to!ushort(head.munch("01234567890"));
                    head.munch(". ");
                    p.status.code = to!ushort(head.munch("01234567890"));
                    p.status.reason = head.idup;
            ...
            auto fieldName = m[0].idup;
            if (fieldName == "content-type")
            {
                if (m[1].canFind("="))
                    p.charset = m[1].split("=")[1].idup;
            }

            p.headersIn[fieldName] = m[1].idup;
        };
